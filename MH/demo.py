...
if choice == "메인페이지":
    ...
    with tab2:
        ...
        def main():
            ...
            options = ["데이터 추가➕", "데이터 조회🔎", "데이터 초기화✂", "노블 사용⭕or제한❌", "위클리 코젬 계산📋", "데이터 다운로드💾", "데이터 삭제"]
            ...
            if option == "데이터 추가➕":
                ...
                if st.button('Delete Data'):
                    ...
                    de_name = row_index >= 0 and row_index < data.shape[0]
                # 해당 행이 존재할 경우, 행을 삭제
                
                    if row_index >= 0 and row_index < data.shape[0]:
                        if not de_name:
                            st.warning('입력하신값이 없습니다.')
                        else : 
    ...
    tab1, tab2= st.tabs(["🎁Random Box", "🔗Link"])
    with tab1:
